...
def start_screen(screen, clock):
    ...
    screen.blit(fon, (0, 0))
    # draw play button
    play = load_image(('start_screen_images', 'play_btn.png'))
    screen.blit(play, (229, 100))
    # draw options button
    options = load_image(('start_screen_images', 'options_btn.png'))
    screen.blit(options, (229, 220))
    # draw exit button
    exit_ = load_image(('start_screen_images', 'exit_btn.png'))
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.MOUSEMOTION:
                ...
                if 229 < x < 229 + 343 and 100 < y < 100 + 91:
                    play = load_image(('start_screen_images', 'play_btn_act.png'))
                else:
                    play = load_image(('start_screen_images', 'play_btn.png'))
                screen.blit(play, (229, 100))

                if 229 < x < 229 + 343 and 220 < y < 220 + 91:
                    options = load_image(('start_screen_images', 'options_btn_act.png'))
                else:
                    options = load_image(('start_screen_images', 'options_btn.png'))
                screen.blit(options, (229, 220))

                if 229 < x < 229 + 343 and 340 < y < 340 + 91:
                    exit_ = load_image(('start_screen_images', 'exit_btn_act.png'))
                else:
                    exit_ = load_image(('start_screen_images', 'exit_btn.png'))
                screen.blit(exit_, (229, 340))
